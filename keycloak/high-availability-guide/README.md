...
> 이 저장소는 EKS에서 Keycloak 단일 클러스터 이중화를 설명하기 위한 학습용 샘플입니다. 운영 환경에 바로 적용하기 전에는 반드시 조직의 보안, 네트워크, 데이터베이스, 배포 정책에 맞게 검토해야 합니다.
...
실제 배포 전에는 `{{PLACEHOLDER}}` 값을 바꾸고, 클러스터의 스케줄링 정책과 데이터베이스 구성을 반드시 검토하세요.

## 범위

이 저장소의 초점은 **single-cluster HA**입니다.
...
Keycloak 이중화는 단순히 replica 수를 늘리는 문제가 아닙니다. 세션, 캐시, 데이터베이스, 로드밸런서, 배포 전략을 함께 봐야 합니다. 설치 구조가 아직 익숙하지 않다면 먼저 `helm-practical-guide` 예제를 보고, 로그인 세션이 왜 중요한지 헷갈린다면 인증 흐름 글을 먼저 읽는 편이 좋습니다.
